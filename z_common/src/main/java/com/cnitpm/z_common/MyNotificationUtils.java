...
public class MyNotificationUtils {
    ...
    public static NotificationCompat.Builder showNotificationProgressApkDown(Context mContext, String title,String name,int progress) {
        final NotificationCompat.Builder builder = getNotificationBuilder(mContext,title,name,"yunjia");
        builder.setOnlyAlertOnce(true);
        builder.setDefaults(Notification.FLAG_ONLY_ALERT_ONCE);
        builder.setProgress(100, progress, false);
        builder.setWhen(System.currentTimeMillis());
        getManager(mContext).notify(R.mipmap.logoc, builder.build());
        builder.setProgress(100, 70, false);
        return builder;
    }
    public static void setProgress(int progress,NotificationCompat.Builder builder){
        builder.setProgress(100, progress, false);
        manager.notify(R.mipmap.logoc, builder.build());
    }

    public static void setContentText(String string,NotificationCompat.Builder builder){
